...
from tests.test_utils import set_point, verify_bet_not_on_table, verify_bet_on_table, verify_no_outcome, \
    verify_roll_loses, verify_roll_wins
...
def test_place_bet():
    ...
    player.bet(bet)
    verify_no_outcome(table, player, (2, 3))
    verify_bet_on_table(player, bet)
...
def test_odds():
    ...
    player = Player(bankroll=bankroll)
    player.bet(passline_bet)
    verify_no_outcome(table, player, (3, 6))
    verify_bet_on_table(player, passline_bet)
    player.add_odds(passline_bet, unit)
    verify_no_outcome(table, player, (1, 4))
    verify_no_outcome(table, player, (2, 3))
    verify_bet_on_table(player, passline_bet)
    verify_bet_on_table(player, player.get_bet_type(Odds))

    dice = Dice()
    dice.fixed_roll((4, 5))
    info = player._update_bet(table, dice)
    assert [result.status == BetStatus.WIN for (name, result) in info.items()]
